 ...
 * 任期の開始日・終了日（比較に使う形）。
 *
 * COUNCIL_TERM_LABEL は画面に出す文字列なので、日付の大小を比べる用途には使えない。
 * 「今の任期に開かれた定例会だけを並べる」といった絞り込みにはこちらを使う。
 */
export const COUNCIL_TERM_START_DATE = "2023-01-24";
export const COUNCIL_TERM_END_DATE = "2027-01-23";

/** 任期の途中で議員でなくなった人 */
export type FormerCouncilMember = {
  /** 氏名から空白を除いたもの（toQuestionerSlug と同じ規則） */
  slug: string;
  name: string;
  /** 議員でなくなった日（YYYY-MM-DD）。この日より後の定例会は在任期間外として扱う */
  leftOn: string;
  /** 画面に出す表記（例: 令和6年12月） */
  leftOnLabel: string;
};
 ...
 *
 * 【なぜ必要か】
 * 定例会ごとの登壇状況を任期全体で並べると、退任後の定例会も表のマスとして
 * 残る。そこを他の議員と同じ「登壇なし」で埋めると、議員でなかった期間まで
 * 「質問しなかった」と読まれてしまう。在任していない期間だと分かるように、
 * 退任日を持っておく。
 *
 * 【運用】
 * 議員でなくなった事実と日付は、市が公開している資料（市議会だより・市議会
 * ホームページ）で確認できたものだけを書く。理由（辞職・失職など）は
 * ここには書かない。掲載の目的は在任期間の判定であり、経緯の説明ではないため。
 *
 * 現在は空にしている。令和5年3月定例会からの一般質問を取り込む段階で、
 * 任期途中で退任した議員の退任日を出典付きで確認してから追加する。
 */
export const FORMER_COUNCIL_MEMBERS: FormerCouncilMember[] = [];
