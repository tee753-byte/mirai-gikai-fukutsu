...
import type { RelatedBillsResult } from "../../loaders/get-related-bills";

interface RelatedBillsSectionProps {
  related: RelatedBillsResult | null;
}

/**
 * 同じ内容で提出された、ほかの会期の議案への導線。
 *
 * 会議録や市議会だよりが公開されるまで数か月かかるため、直近の会期の
 * ページは中身が薄い。同じ議案が過去にも出ていれば、そちらでは討論や
 * 議員別の賛否まで読めるので、そこへ渡す。
 *
 * 議決結果のバッジを出すのは、同じ議案が繰り返し出されていること自体が
 * 市民にとっての情報だから。どちらが正しいという書き方はしない。
 */
export function RelatedBillsSection({ related }: RelatedBillsSectionProps) {
  if (!related || related.bills.length === 0) {
    return null;
  }
  ...
  return (
    <section className="bg-card rounded-xl p-6">
      ...
      <p className="text-sm leading-relaxed text-mirai-text-secondary mb-5">
        {related.description}
        会議録が公開されている会期では、質疑・討論や議員ごとの賛否まで読めます。
      </p>

      <ul className="flex flex-col divide-y divide-mirai-border">
        {related.bills.map((bill) => (
          <li key={bill.id}>
            <Link
              href={`/bills/${bill.id}`}
