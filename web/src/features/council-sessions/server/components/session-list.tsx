 ...
 * その会期がどこまで反映済みかの一文。
 *
 * 資料は3段階で出てくる。議決結果は会期の直後、会議録はおよそ2〜3か月後、
 * 議員別の賛否が載る市議会だよりはおよそ2か月後である。
 * そのため直近の会期は必ず「議決結果まで」の状態になり、そのあと
 * 「会議録まで」を経て「賛否まで」に揃う。
 * 完成度の違いを隠して並べると読み手が誤解するので、カードに明示する。
 *
 * 会議録が入ると討論と提案理由の説明が読めるようになるため、
 * 「討論は会議録の公開後に追加します」のままにしておくと事実と食い違う。
 */
function coverageLabel(summary: SessionSummary): string {
  if (summary.hasMemberVotes) return "会議録と議員別の賛否まで反映済み";
  if (summary.hasMinutes) {
    return "会議録まで反映済み。議員別の賛否は、市議会だよりの公開後に追加します";
  }
  return "議決結果まで掲載。討論と議員別の賛否は、会議録と市議会だよりの公開後に追加します";
}
