...
export type BillContentInput = {
   ...
   * 理由がそもそも記録されていないものは undefined にする。
   */
  reasonPlain?: string | null;
  /**
   * 理由の記録が無い議案（予算など）に置く、しくみの説明。
   *
   * 予算の議案には議案書に理由欄が無く、会議録にも提案理由の説明が残って
   * いないことがある。空欄のままだと、資料に無いのか載せ忘れなのかが
   * 読み手に区別できない。
   *
   * reasonPlain とは別の欄にしているのは、出どころが違うため。
   * reasonPlain は市や提出議員の説明を書き直したもの、こちらは
   * 法律や制度そのものの説明で、誰かの説明を引用したものではない。
   * 混ぜると、市が言っていないことを市の説明として読ませてしまう。
   */
...
function reasonSection(input: BillContentInput): string | null {
  // 理由の記録が無い議案は、しくみの説明に置き換える。
  // 誰かの説明ではないので、見出しも「どんな議案か」に変える
  if (!input.reasonPlain) {
    return input.systemNote ? `## どんな議案か\n\n${input.systemNote}` : null;
  }
