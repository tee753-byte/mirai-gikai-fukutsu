...
- サブセット化時にグリフ番号が元の MS ゴシック／明朝のまま維持されているため、
  ...
  実測で復号率98.9%（`(cid:15910)`→`（`、`(cid:7517)`→`様` 等）
...
- **例外**: R5の農林水産部・県土整備部・建築都市部の分冊（716126_62029375）は
  **Type3フォント**（グリフが描画手続きで文字コード対応表が原理的に存在しない）のため、
  この方法では復号不可。OCR以外に手段がない
...
| R5以前 | ToUnicode 欠落のサブセットフォントで文字が `(cid:N)` でしか取れない | 復号は可能（上記「R5以前のCID復号」）。ただし取り込む価値がないため未実装（上記「過年度を広げても増えない」）。予算推移はR5決算〜R8当初の4年分で確定 |
